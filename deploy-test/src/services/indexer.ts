...
import { Mem9 } from '@cortex/shared-mem9'
...
export async function startIndexing(projectId: string, jobId: string, branch: string): Promise<void> {
  ...
  try {
    ...
    // ── Step 3: mem9 Ingest (branch-scoped) ──
    updateJob(jobId, { status: 'ingesting', progress: 80 })
    logger.info(`[${jobId}] Ingesting to mem9 (in-process)`)

    let mem9Status = 'skipped'
    try {
      const mem9 = new Mem9({
        llm: {
          baseUrl: process.env.LLM_PROXY_URL || 'http://llm-proxy:8317/v1',
          model: process.env.MEM9_LLM_MODEL || 'gpt-5.4-mini',
        },
        embedder: {
          provider: 'gemini',
      ...
      const branchResult = await mem9.add({
      ...
      const baseResult = await mem9.add({
        messages: [{ role: 'user', content: `[branch:${branch}] ${summary}` }],
        userId: baseUserId,
        metadata,
      })

      const totalEvents = branchResult.events.length + baseResult.events.length
      mem9Status = totalEvents > 0 ? 'ok' : 'no-new-facts'
      appendLog(jobId, `✅ mem9 ingest OK (${totalEvents} events, branch: ${branchUserId}, base: ${baseUserId})`)
    } catch (err) {
      mem9Status = 'failed'
      appendLog(jobId, `❌ mem9 ingest failed: ${err}`)
      logger.warn(`[${jobId}] mem9 ingest failed (non-fatal): ${err}`)
    }
