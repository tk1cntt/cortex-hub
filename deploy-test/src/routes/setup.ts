...
setupRouter.post('/complete', async (c) => {
  try {
    ...
    let mem9Status: 'ok' | 'degraded' | 'error' = 'error'
    try {
      const qdrantUrl = process.env.QDRANT_URL || 'http://qdrant:6333'
      const llmUrl = process.env.LLM_PROXY_URL || 'http://llm-proxy:8317'

      const [qdrantRes, llmRes] = await Promise.all([
        fetch(`${qdrantUrl}/healthz`, { signal: AbortSignal.timeout(5000) }),
        fetch(`${llmUrl}/v1/models`, { signal: AbortSignal.timeout(5000) }),
      ])

      if (qdrantRes.ok && llmRes.ok) {
        mem9Status = 'ok'
        console.log('[Setup] mem9 dependencies healthy (Qdrant + CLIProxy)')
      } else {
        mem9Status = 'degraded'
        console.warn(`[Setup] mem9 degraded: qdrant=${qdrantRes.status} cliproxy=${llmRes.status}`)
      }
    } catch (err) {
      console.warn('[Setup] mem9 dependency check failed:', err)
      mem9Status = 'error'
    }
    ...
    return c.json({
      ...
      mem9: {
        ...
        message:
          mem9Status === 'ok'
            ? 'mem9 memory engine is ready (Qdrant + CLIProxy + Gemini)'
            : 'mem9 dependencies not fully available — they will auto-connect when containers start',
      },
    })
  } catch (error) {
...
setupRouter.post('/configure-mem9', async (c) => {
  try {
    ...
    // Store the key as env var (in production this would be saved to a config store)
...
setupRouter.get('/settings', (c) => {
  return c.json({
    ...
    services: {
      ...
      qdrant: QDRANT_URL(),
      mem9: 'in-process (@cortex/shared-mem9)',
      geminiEmbedding: process.env.GEMINI_API_KEY ? 'configured' : 'not-configured',
