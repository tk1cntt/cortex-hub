...
app.get('/health', async (c) => {
  ...
  // mem9 readiness = qdrant + cliproxy (embedding is external Gemini API, not checked here)
  const mem9 = qdrant === 'ok' && cliproxy === 'ok' ? 'ok' : 'degraded'

  const services = { qdrant, cliproxy, mem9 }
  const allOk = qdrant === 'ok' && cliproxy === 'ok'
